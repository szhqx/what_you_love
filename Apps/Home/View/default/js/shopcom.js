...
function toViewGoods(id){
	$.post(Think.U('Home/Goods/getGoodsVerify'),{id:id},function(data,textStatus){
		...
		if(json.status=='1'){
			...
			window.open(Think.U('Wx/Goods/getGoodsDetails','goodsId='+id+"&kcode="+verifyCode));
		}
	});
		...
		if(params.priceAttrId>0){
			 ...
			 for(var i=0;i<=params.goodsPriceNo;i++){
				 if(document.getElementById('price_name_'+params.priceAttrId+'_'+i)){
					 ...
					 params['price_lowprice_'+params.priceAttrId+'_'+i] = $.trim($('#price_lowprice_'+params.priceAttrId+'_'+i).val());
...
function addPriceAttr(){
	...
	html.push('<td><input type="text" id="price_lowprice_'+obj.attrId+'_'+goodsPriceNo+'" value="0" onblur="checkAttPrice('+obj.attrId+','+goodsPriceNo+');" onkeypress="return WST.isNumberdoteKey(event)" onkeyup="javascript:WST.isChinese(this,1)" maxLength="10"/></td>');
	if(goodsPriceNo==1){
