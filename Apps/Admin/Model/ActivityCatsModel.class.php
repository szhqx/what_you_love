 ...
 * 文章分类服务类
 */
class ActivityCatsModel extends BaseModel {
    /**
	  * 新增
	  */
     ...
     public function queryByPage(){
	 	$sql = "select * from __PREFIX__activity_cats order by catSort asc";
	 ...
	 public function getChild($ids = array(),$pids = array()){
	 	$sql = "select catId from __PREFIX__activity_cats where catFlag=1 and parentId in(".implode(',',$pids).")";
	 	$rs = $this->query($sql);
	 	if(count($rs)>0){
	 		$cids = array();
		 	foreach ($rs as $key =>$v){
	 ...
	 public function getCatLists(){	
	 	$sql = "select * from __PREFIX__activity_cats where catFlag = 1 order by catSort asc";
