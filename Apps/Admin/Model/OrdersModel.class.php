...
class OrdersModel extends BaseModel {
	 ...
	 */
	  ...
	  */
	 public function get(){
	 	return $this->where('isRefund=0 and payType<>0  and orderFlag=1 and orderStatus in (-1,-4,-6,-7) and orderId='.(int)I('id'))->find();//and isPay=1
	 }
	 /**
	  * 订单分页列表
	  */
     ...
     public function queryRefundByPage(){
        ...
        $shopName = WSTAddslashes(I('shopName'));
	 	...
	 	$sql = "select o.orderId,o.orderNo,o.totalMoney,o.orderStatus,o.isRefund,o.deliverMoney,o.payType,o.createTime,s.shopName,o.userName from __PREFIX__orders o
	 	         left join __PREFIX__shops s on o.shopId=s.shopId  where o.orderFlag=1 and o.orderStatus in (-1,-4,-6,-7) and payType<>0 ";//and isPay=1
	 	if($areaId1>0)$sql.=" and s.areaId1=".$areaId1;
	 	if($areaId2>0)$sql.=" and s.areaId2=".$areaId2;
	 	if($areaId3>0)$sql.=" and s.areaId3=".$areaId3;
	 ...
	 public function refund(){
	 	$rd = array('status'=>-1);
	 	$rs = $this->where('isRefund=0 and orderFlag=1 and orderStatus in (-1,-4,-6,-7) and payType<>0 and orderId='.(int)I('id'))->find();//and isPay=1
	 	if($rs['orderId']!=''){
	 		$data = array();
	 		$data['isRefund'] = 1;
