	 ...
	 */
	 public function get(){
		...
		if($goods['attrCatId']>0){
			...
			//获取规格属性
			$sql = "select ga.attrVal,ga.attrlowPrice,ga.profitPrice,ga.id as attributesId,ga.attrPrice,ga.attrStock,a.attrId,a.attrName,a.isPriceAttr,ga.isRecomm
	 ...
	 public function changeGoodsStatus(){
	 	 $rd = array('status'=>-1);
	 	 $id = (int)I('id',0);
	 	 $data['goodsStatus'] = (int)I('status',0);
		 $post_data = I();
		 foreach($post_data as $key=>$val){
		 	if(strpos($key,"profitPrice") !== false){     //使用绝对等于
				if($post_data[$key]=='0.00') {return $rd;exit;}
			}
		 }
		 $next = 1; //用户判断有无更新失败
		 foreach($post_data as $key=>$val){
			 if(strpos($key,"profitPrice") !== false){     //使用绝对等于
				 if($key == 'profitPrice'){
					 $data['profitPrice'] = $post_data[$key];
				 }else{
					 $temp = array();
					 $temp = explode("_",$key);
					 $at =  M("goods_attributes")->where("goodsId=".$id." AND id=".$temp[1])->save(array("profitPrice"=>$post_data[$key]));
					 if($at === false){
						 $next = '-1';
					 }
				 }
		...
		if($next =='-1'){return $rd;exit;}
		$rs = $this->where('goodsId='.$id)->save($data);
		if(false !== $rs){
