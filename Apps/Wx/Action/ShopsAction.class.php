...
//		$this->assign('shopInfo',$data);
		...
		$spm = D('Wx/Shops');
		$sql = "SELECT count(*) cnt FROM __PREFIX__favorites WHERE favoriteType = 1 AND targetId = $shopId AND userId=$userId";
		$favoriteType = $spm->queryRow($sql);
		if($favoriteType['cnt'] != 0){//已关注
			$favoriteType = $shopId;
		}else{
			$favoriteType = 0;
		}

		$this->assign("favoriteType",$favoriteType);//关注
		$this->assign('shopId',$shopId);
		$spm = D('Wx/Shops');
		$details = $spm->getShopDetails($obj);
		$shopdata = $spm->getShopInfo($shopId);
		$data['details'] = $details;
		$this->assign("shop",$shopdata);
		$this->assign('shopInfo',$data['details']);
