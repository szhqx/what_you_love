...
class GoodsAction extends BaseAction {
	...
	public function getMore(){
		...
		$rslist = $mgoods->getGoodsList();
		$pages = $rslist["pages"];
		print_r(json_encode($pages['root']));
		exit;
	}
	...
	public function getGoodsDetails(){
		...
		$typeId= 0;
		$goods->setFootPrint($typeId,$goodsId);
		...
		if($kcode==$scrictCode || ($goodsDetails["isSale"]==1 && $goodsDetails["goodsStatus"]==1)){
			...
			$goodsDetails['goodsParameter'] = htmlspecialchars_decode($goodsDetails['goodsParameter']);
			...
			//无价格的商品规格
			foreach($goodsAttrs['attrs'] as $key=>$val){
				$goodsAttrs['attrs'][$key]['attrContent'] = explode(',',$val['attrContent']);
			}
			...
			//获取商品评论
			$ms = D('Wx/GoodsAppraises');
			$appraises = $ms->getGoodsAppraise($goodsId);
		   ...
		   // $this->assign("qrcode",base64_encode("{type:'goods',content:'".$goodsId."',key:'wstmall'}"));
			...
			$this->display('goods/goods_details');
		}else{
			showmsg("商品已下架或禁售");
			$this->display('goods/goods_notexist');
		}
