...
class GoodsModel extends BaseModel {
	...
	public function setFootPrint($typeId=0,$objectId = ''){
		$userId = (int)session('WST_USER.userId');
		if($userId){ //用户存在才存，避免数据库插入过多数据
			$data = M("user_footprint")->where("userId ='$userId' AND typeId='$typeId' AND objectId='$objectId'")->find();
			if(!is_array($data)&& M("goods")->where("goodsId=$objectId")->find()){ //没存，且对象存在
				M("user_footprint")->add(array("userId"=>$userId,"typeId"=>$typeId,"objectId"=>$objectId));
			}
		}
	...
	public function getGoodsList(){
		...
		$sql .= "WHERE  g.shopId = p.shopId AND  g.goodsStatus=1 AND g.goodsFlag = 1 and g.isSale=1 ";
		...
		$pages = $this->pageQuery($sql,$pcurr,12);
	...
	public function getMaxPrice($obj){
		...
		$sql .= "WHERE g.shopId = p.shopId AND bd.brandId=g.brandId AND  g.goodsStatus=1 AND g.goodsFlag = 1";
		if($c1Id>0){
			$sql .= " AND g.goodsCatId1 = $c1Id";
		}
		if($c2Id>0){
	...
	public function getGoodsSimpInfo($goodsId,$goodsAttrId){
		...
		$rs = $this->queryRow($sql);
	    if(!empty($rs) && $rs['attrCatId']>0){
        	$sql = "select ga.id,ga.attrPrice,ga.attrStock,a.attrName,ga.attrVal,ga.attrId,ga.attrlowPrice,ga.profitPrice from __PREFIX__attributes a,__PREFIX__goods_attributes ga
			        where a.attrId=ga.attrId and a.catId=".$rs['attrCatId']." 
			        and ga.goodsId=".$rs['goodsId']." and id=".$goodsAttrId;
			$priceAttrs = $this->queryRow($sql);
			if(!empty($priceAttrs)){
				...
				$rs['shopPrice'] = $priceAttrs['attrPrice'];
				$rs['goodsStock'] = $priceAttrs['attrStock'];
				$rs['attrlowPrice'] = $priceAttrs['attrlowPrice'];
				$rs['attrprofitPrice'] = $priceAttrs['profitPrice'];
			}
        }
        $rs['goodsAttrId'] = (int)$rs['goodsAttrId'];
		return $rs;
