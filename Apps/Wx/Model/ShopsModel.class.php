    ...
    */
	...
	/**
	 ...
	 public function addByVisitor(){
		...
		$shopRules = array(
			...
			array('charterThums','require','请上传营业执照!',1),
	...
	public function getShopInfo($oshopId = 0){
		...
		$sql = "SELECT g.saleCount, g.shopId , g.goodsId , g.goodsName,g.goodsImg, g.goodsThums,g.shopPrice,g.marketPrice, g.goodsSn 
						...
						ORDER by g.saleCount ";
		$hotgoods = $this->query($sql);
		$rs["hotgoods"] = $hotgoods;
		/**** 获取店铺分类及分类下商品 *****/

		$sql = "select catId,catName from __PREFIX__shops_cats WHERE parentId = 0 AND isShow =1 AND catFlag = 1 AND shopId=$shopId order by catSort asc";
		$rs1 = $this->query($sql);
		$cats = array();
		for ($i = 0; $i < count($rs1); $i++) {
			$cat1Id = $rs1[$i]["catId"];
				...
				$sql = "SELECT sp.shopName, g.saleCount , sp.shopId , g.goodsId , g.goodsName,g.goodsImg, g.goodsThums,g.shopPrice, g.goodsSn,ga.id goodsAttrId,ga.attrPrice
							...
							WHERE g.shopId = sp.shopId AND sp.shopStatus = 1 AND g.isAdminBest <> 1 AND g.goodsFlag = 1 AND g.isSale = 1 AND g.goodsStatus = 1 AND g.shopCatId1 = $cat1Id
...
//		$data["waitSaleGoodsCnt"] = $goods["cnt"];
		...
		//关注店铺数
		$sql = "SELECT count(*) cnt FROM __PREFIX__favorites WHERE favoriteType = 1 AND targetId = $shopId";
		$shops = $this->queryRow($sql);
		$data["onFavoriteCnt"] = $shops["cnt"];
		//买家留言
