...
class UsersModel extends BaseModel {
	 ...
	 * 获取余额明细
	 */
	public function getPurseDetails($obj){
		$userId = (int)$obj["userId"];
		$ua = M('users_amount');
		$data = $ua->where('userId='.$userId)->order("createTime desc")->select();
		return $data;
	}
	...
	/**
     ...
     public function getUserById($obj){
		$userId = (int)$obj["userId"];
		 ...
		 $rs["partnerName"] = '';
		 if($rs['partner']>0){
			 $partnerName = $this->where("userId=".$rs['partner'])->find();
			 $rs["partnerName"] = empty($partnerName["nickName"])?$partnerName["userName"]:$partnerName["nickName"];
		 }
	    return $rs;
	 }
	...
	public function loginByOpenid($openid){
		if(!$openid)
			return;
		$account =$this->where(array("openid" => $openid))->find();
		if(!empty($account)){
			$rs = $account;
			$data = array();
			$data['lastTime'] = date('Y-m-d H:i:s');
			...
			$accname = $account['loginName'];
			$security = md5('xhs-'.$accid.'-'.$accname.'-'.$_SERVER['HTTP_USER_AGENT']);
			session('userId', $accid);
			session('loginName', $accname);
			cookie('userId', authcode($accid, 'ENCODE'), 2592000);
			setcookie("loginName", $accname, time()+3600*24*60);
			cookie('loginName', authcode($accname, 'ENCODE'), 2592000);
			cookie('security', authcode($security, 'ENCODE'), 2592000);
			session('WST_USER',$rs);
		}
		return $account;
	}
	...
	public function getMyFootPrint(){
		...
		$sql = "select f.Id,g.goodsId,g.goodsThums,g.goodsName,g.shopPrice
			from __PREFIX__user_footprint f ,__PREFIX__goods g
			where f.objectId = g.goodsId
			and f.typeId=0 and f.userId=".(int)session('WST_USER.userId');
