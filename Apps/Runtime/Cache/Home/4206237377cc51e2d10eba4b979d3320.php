    ...
    <div class="wst-body"> 
       ...
       <div class='wst-page-content'>
       	<div style="padding:6px;">
       		...
       		<button class="wst-btn-query" onclick="javascript:getOrdersList(4)">查询</button>
       	</div>
        <table class='wst-list' style="font-size:13px;">
           ...
           <tbody>
            <?php if(is_array($appraiseOrders['root'])): $key1 = 0; $__LIST__ = $appraiseOrders['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($key1 % 2 );++$key1;?><tr height="60">
                ...
                <td width='*'>
					<?php if(is_array($order['goodslist'])): $key2 = 0; $__LIST__ = $order['goodslist'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($key2 % 2 );++$key2;?><a target='_blank' href="<?php echo U('Wx/Goods/getGoodsDetails/',array('goodsId'=>$goods['goodsId']));?>">
				...
				<td width='100' style="padding-top:10px;vertical-align:top;"><a href="<?php echo U('Home/Shops/toShopHome/',array('shopId'=>$order['shopId']));?>" target="_blank"><?php echo ($order["shopName"]); ?></a></td>
               	...
               	<td width='130' style="padding-top:10px;vertical-align:top;">
					...
					<?php if($order['complainId'] == ''): ?><a href='<?php echo U("Home/OrderComplains/complain",array("orderId"=>$order["orderId"]));?>'>投诉</a><?php endif; ?>
				</td>
