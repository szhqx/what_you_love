        ...
        <table class='wst-list' style="font-size:13px;">
           ...
           <tbody>
            <?php if(is_array($Page['root'])): $key1 = 0; $__LIST__ = $Page['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($key1 % 2 );++$key1;?><tr>
				...
				<td style="padding-top:10px;vertical-align:top;"><a href="<?php echo U('Home/Shops/toShopHome/',array('shopId'=>$order['shopId']));?>" target="_blank"><?php echo ($order["shopName"]); ?></a></td>
               	<td><?php echo WSTMSubstr($order["complainContent"],0,32);?></td>
               	...
               	<td>
					<a href="<?php echo U('Home/OrderComplains/getUserComplainDetail',array('id'=>$order['complainId']));?>">查看</a>
				</td>
