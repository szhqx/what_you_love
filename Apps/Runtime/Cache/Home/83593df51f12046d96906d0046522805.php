...
<div id="wst-shortcut">
    ...
    <div class="wst-body"> 
       <div class='wst-page-header'>卖家中心 > <?php echo ($cat['catName']); ?> > 商品属性</div>
       <div class='wst-page-content'>
        <div class='wst-tbar-group'>
           <div style='float:left;margin-top:8px;margin-left:5px;'>
                                 商品类型：
           <select id="catId" onchange='javascript:getAttrsForAttr(this.value)' dataNo="<?php echo (count($List)); ?>" autocomplete="off">
             <?php if(is_array($catList)): $i = 0; $__LIST__ = $catList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vo["catId"]); ?>' <?php if($cat['catId'] ==$vo['catId']): ?>selected<?php endif; ?>><?php echo ($vo["catName"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
           </select>
           </div>
        ...
        <table class='wst-list'>
           ...
           <tbody id='tbody'>
            <?php if(is_array($List)): $i = 0; $__LIST__ = $List;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id='tr_<?php echo ($i); ?>' dataId='<?php echo ($vo['attrId']); ?>'>
               <td><?php echo ($i); ?></td>
               <td><input type='text' id='attrName_<?php echo ($i); ?>' value="<?php echo ($vo['attrName']); ?>" autocomplete="off"/></td>
               <td><input type='radio' name='isPriceAttr' id='isPriceAttr_<?php echo ($i); ?>' autocomplete="off" <?php if($vo['isPriceAttr'] ==1): ?>checked<?php endif; ?> onclick='javascript:chkPriceAttrForAttr()' id='isPriceAttr_<?php echo ($i); ?>'></td>
               <td>
               <select id='attrType_<?php echo ($i); ?>' onchange='javascript:changeAttrTypeForAttr(<?php echo ($i); ?>)' autocomplete="off" <?php if($vo['isPriceAttr'] ==1): ?>style='display:none'<?php endif; ?>>
	               <option value='0' <?php if($vo['attrType'] ==0): ?>selected<?php endif; ?>>输入框</option/>
	               <option value='1' <?php if($vo['attrType'] ==1): ?>selected<?php endif; ?>>多选项</option/>
	               <option value='2' <?php if($vo['attrType'] ==2): ?>selected<?php endif; ?>>下拉项</option/>
               </select>
               </td>
               <td><input type='text' id='attrContent_<?php echo ($i); ?>' value="<?php echo ($vo['attrContent']); ?>" autocomplete="off" style='width:300px;<?php if($vo['attrType'] ==0 or $vo['isPriceAttr']==1): ?>display:none<?php endif; ?>'/></td>
               <td><input type='text' id='attrSort_<?php echo ($i); ?>' value="<?php echo ($vo['attrSort']); ?>" autocomplete="off"/></td>
               <td>
               <a href="javascript:delAttrs(<?php echo ($i); ?>,<?php echo ($vo['attrId']); ?>)" class='btn del' title='删除'></a>
               &nbsp;
               </td>
        ...
        <div class='wst-tbar-group'>
           <button class='wst-btn-query' style='<?php if( count($List) == 0): ?>display:none;<?php endif; ?>margin-top:5px;margin-left:400px;' type="button" onclick='javascript:editAttrs()'>保&nbsp;存</button>
           <button class='wst-btn-query' style='<?php if( count($List) == 0): ?>display:none;<?php endif; ?>margin-top:5px;margin-left:5px;' type="button" onclick='javascript:getAttrsForAttr()'>取&nbsp;消</button>
           <a style='float:right;margin-right:5px;' href='javascript:toAddAttr()'><span class='add btn'></span>新增</a>&nbsp;
        </div>
        </div>
    </div>
