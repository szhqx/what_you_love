...
<div class="yj-full-body" style="overflow: scroll;position:relative;  -webkit-overflow-scrolling: touch;">
    <form action="/wx/users/release_activity" method="post"  enctype="multipart/form-data">
        ...
        <select name="catId" >
            <?php if(is_array($actCate)): $i = 0; $__LIST__ = $actCate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["catId"]); ?>"><?php echo ($vo["catName"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
        <select name="isshow" >
        ...
        <input type='datetime-local' id='start_time' name='start_time' class="wstipt wst-ipt " style="width: 52%;" value='<?php echo ((isset($object["end_apply"]) && ($object["end_apply"] !== ""))?($object["end_apply"]):"2016-01-01 00:00"); ?>'/>
        <!--js-datetime-->
        <span style="width: 38%;float: left;line-height: 35px;">活动结束时间：</span>
        <input type='datetime-local' id='end_time' name='end_time' class="wstipt wst-ipt " style="width: 52%;" value='<?php echo ((isset($object["end_apply"]) && ($object["end_apply"] !== ""))?($object["end_apply"]):"2016-01-01 00:00"); ?>'/>
        <span style="width: 38%;float: left;line-height: 35px;">报名结束时间：</span>
        <input type='datetime-local' id='end_apply' name='end_apply' class="wstipt wst-ipt " style="width: 52%;" value='<?php echo ((isset($object["end_apply"]) && ($object["end_apply"] !== ""))?($object["end_apply"]):"2016-01-01 00:00"); ?>'/>
        <div style="width: 100%;">
            <div class="uc_form_txt">
