...
<div class="pager">
    <div class="bxcont">
        <div class="purse">
        <div class="purse_tit"><span>收益</span><b><?php echo ((isset($userIncome) && ($userIncome !== ""))?($userIncome):'0.00'); ?></b></div>
            </div>
        <?php if(!empty($incomeList)): ?><div class="msglist">
                    <?php if(is_array($incomeList["root"])): $i = 0; $__LIST__ = $incomeList["root"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="msgitem">
                            <div class="mtit">您通过<span>[<?php echo ($vo["orderunique"]); ?>]</span>分成获得<?php echo ($vo["price"]); ?>元</div>
                            <!--<span hidden>【未读】</span>-->
                            <div class="mtime"><?php echo ($vo["active_time"]); ?></div>
                        </div><?php endforeach; endif; else: echo "" ;endif; ?>
                </div>
         <?php else: ?>
                 <div class="pempty" style="display:block;"><img src="/static/wx/bx/images/cartoon.png"/><br/>暂无明细</div><?php endif; ?>
        <!--消息记录为空时显示-->
