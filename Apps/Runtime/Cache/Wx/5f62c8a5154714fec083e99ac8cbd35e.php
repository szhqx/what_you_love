      ...
      <div id="wrapper">
          <div id="viewport">
              <div class="order_list">
                  <?php if(is_array($orderList['root'])): $key1 = 0; $__LIST__ = $orderList['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($key1 % 2 );++$key1;?><div id="OrderList" class="ord_list">
                          <h3>订单流水号：<?php echo ($order[0]["orderunique"]); ?><span style="float: right; padding-right: 10%;"><var style="color: #999;"> 状态：</var><?php if($order[0]["orderStatus"] == -3): ?>拒收
                              <?php elseif($order[0]["orderStatus"] == -2): ?>未付款
                              <?php elseif($order[0]["orderStatus"] == -1): ?>已取消
                              <?php elseif($order[0]["orderStatus"] == 0): ?>未受理
                              <?php elseif($order[0]["orderStatus"] == 1): ?>已受理
                              <?php elseif($order[0]["orderStatus"] == 2): ?>打包中
                              <?php elseif($order[0]["orderStatus"] == 3): ?>配送中
                              <?php elseif($order[0]["orderStatus"] == 4): ?>已到货
                              <?php elseif($order[0]["orderStatus"] == 5): ?>确认收货<?php endif; ?></span></h3>
                          <?php if(is_array($order)): $i = 0; $__LIST__ = $order;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ods): $mod = ($i % 2 );++$i; if(is_array($ods['goodslist'])): $key2 = 0; $__LIST__ = $ods['goodslist'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$goods): $mod = ($key2 % 2 );++$key2;?><dl class="dingdan">
                          ...
                          <ul>
                              <li>总价<strong>：￥
                                  <?php
 $countMoney = 0; foreach($order as $key=>$val){ $countMoney +=$val['totalMoney']; } $deliverMoney=0; if($val['totalMoney']>0){ $deliverMoney += $val['deliverMoney']; } echo $countMoney; ?>元</strong> </li>
                              <li>
                                  <?php if($order[0]["orderStatus"] == -3): ?><a href="javascript:;">已取消</a>
                                      <a href="javascript:;" onclick="showOrder('<?php echo ($order["orderId"]); ?>')">查看</a>
                                      <?php elseif($order[0]["orderStatus"] == -2): ?>
                                      <a href="javascript:;" onclick="toPay(<?php echo ($order[0]['orderunique']); ?>,-2)">去支付</a>
                                      <a href="javascript:;" onclick="showOrder(<?php echo ($order[0]["orderId"]); ?>)">&nbsp;查&nbsp;&nbsp;看&nbsp;</a>
                                      <a href="javascript:;" onclick="orderCancel(<?php echo ($order[0]['orderunique']); ?>,-2)">取消订单</a>
                                      <?php elseif($order[0]["orderStatus"] == -1): ?>
                                      <a href="javascript:;">已取消</a>
                                      <a href="javascript:;" onclick="showOrder('<?php echo ($order["orderId"]); ?>')">查看</a>
                                      <?php elseif($order[0]["orderStatus"] == 0): ?>
                                      <a href="javascript:;" onclick="orderCancel(<?php echo ($order['orderId']); ?>,<?php echo ($order["orderStatus"]); ?>)">取消订单</a>
                                      <a href="javascript:;" onclick="showOrder('<?php echo ($order["orderId"]); ?>')">查看</a>
                                      <?php elseif($order[0]["orderStatus"] == 1): ?>
                                      <a href="javascript:;" onclick="orderCancel(<?php echo ($order['orderId']); ?>,<?php echo ($order["orderStatus"]); ?>)">取消订单</a>
                                      <a href="javascript:;" onclick="showOrder('<?php echo ($order["orderId"]); ?>')">查看</a>
                                      ...
                                      <a href="javascript:;" onclick="orderCancel(<?php echo ($order['orderId']); ?>,<?php echo ($order["orderStatus"]); ?>)">取消订单</a>
                                      <a href="javascript:;" onclick="showOrder('<?php echo ($order["orderId"]); ?>')">查看</a>
                                      <?php elseif($order[0]["orderStatus"] == 3): ?>
                                      <a href="javascript:;" onclick="orderCancel(<?php echo ($order['orderId']); ?>,<?php echo ($order["orderStatus"]); ?>)">取消订单</a>
                                      <a href="javascript:;" onclick="showOrder('<?php echo ($order["orderId"]); ?>')">查看</a>

                                      <?php elseif($order[0]["orderStatus"] == 4): ?>
                                          ...
                                          <a href="javascript:;" onclick="showOrder('<?php echo ($order["orderId"]); ?>')">查看</a>
                                      <?php elseif($order[0]["orderStatus"] == 5): ?>
                                          <?php if($order['isAppraises'] == 1): ?><a  href="javascript:;" >已评价</a>
                                              <?php else: ?>
                                              <a  href="javascript:;" onclick="appraiseOrder(<?php echo ($order['orderId']); ?>)">评价</a><?php endif; ?>
                                          <a href="javascript:;" onclick="showOrder('<?php echo ($order["orderId"]); ?>')">查看</a><?php endif; ?>
