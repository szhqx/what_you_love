...
<html lang="zh-cn">
   ...
   function toEdit(id,pid){
	   var url = "<?php echo U('Admin/ActivityCats/toEdit',array('id'=>'__0','parentId'=>'__1'));?>";
	   url = WST.replaceURL(url,[id,pid]);
       location.href=url;     
   }
   function editName(obj){
	   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
	   $.post("<?php echo U('Admin/ActivityCats/editName');?>",{id:$(obj).attr('dataId'),catName:obj.value},function(data,textStatus){
   ...
   function toggleIsShow(t,v){
	   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
	   $.post("<?php echo U('Admin/ActivityCats/editiIsShow');?>",{id:v,isShow:t},function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
   ...
   function del(id){
	   Plugins.confirm({title:'信息提示',content:'您确定要删除该文章分类吗?',okText:'确定',cancelText:'取消',okFun:function(){
		   Plugins.closeWindow();
		   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
		   $.post("<?php echo U('Admin/ActivityCats/del');?>",{id:id},function(data,textStatus){
					var json = WST.toJson(data);
					if(json.status=='1'){
						Plugins.setWaitTipsMsg({content:'操作成功',timeout:1000,callback:function(){
						   location.reload();
   ...
   function loadChildTree(obj,pid,objId){
		...
		if($(obj).hasClass('glyphicon-minus')){
			...
			if($(obj).attr('isLoad')){
				...
				$.post("<?php echo U('Admin/ActivityCats/queryByList');?>",{id:pid},function(data,textStatus){
					$(obj).attr("isLoad",1);
					var json = WST.toJson(data);
					if(json.list && json.list.length>0){
						...
						for(var i=0;i<json.length;i++){
							...
							html.push("<td>");
							<?php if(in_array('hdfl_01',$WST_STAFF['grant'])){ ?>
							...
							<?php } ?>
							<?php if(in_array('hdfl_02',$WST_STAFF['grant'])){ ?>
							...
							<?php } ?>
							<?php if(in_array('hdfl_03',$WST_STAFF['grant'])){ ?>
							if(json[i].catType==0){
   ...
   <body class='wst-page'>
       ...
       <?php if(in_array('hdfl_01',$WST_STAFF['grant'])){ ?>
       <a class="btn btn-success glyphicon glyphicon-plus" href="<?php echo U('Admin/ActivityCats/toEdit');?>" style='float:right'>新增</a>
       <?php } ?>
       </div>
       <div class="wst-body"> 
        <table class="table table-hover table-striped table-bordered wst-list">
           ...
           <tbody>
            <?php if(is_array($List)): $i = 0; $__LIST__ = $List;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id='tr_0_<?php echo ($i); ?>' class="tr_0">
               ...
               <td>
               <!--<?php if(in_array('hdfl_01',$WST_STAFF['grant'])){ ?>-->
               <!--<a class="btn btn-default glyphicon glyphicon-plus" href="<?php echo U('Admin/ActivityCats/toEdit',array('parentId'=>$vo['catId']));?>">新增子分类</a>&nbsp;-->
               <!--<?php } ?>-->
               <?php if(in_array('hdfl_02',$WST_STAFF['grant'])){ ?>
               <a class="btn btn-default glyphicon glyphicon-pencil" href="<?php echo U('Admin/ActivityCats/toEdit',array('id'=>$vo['catId']));?>">修改</a>&nbsp;
               <?php } ?>
               <?php if(in_array('hdfl_03',$WST_STAFF['grant'])){ ?>
