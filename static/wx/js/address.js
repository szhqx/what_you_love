...
function toEditAddress(id){
	location.href = '/Wx/UserAddress/toEdit','id='+id;
}
...
function changeAddress(addressId){
	...
	if(addressId>=1){
		window.location.href="/Wx/UserAddress/toEdit/id/"+addressId;
		return false;
	}else{
		window.location.href="/Wx/UserAddress/toEdit";
		return false;
	}
}
	...
	jQuery.post('/Wx/UserAddress/edit' ,params,function(data,textStatus){
		...
		if(json.status>0){
			if(addressId==0){
				...
				window.location.href = "/Wx/user_address/querybypage";
			}else{
